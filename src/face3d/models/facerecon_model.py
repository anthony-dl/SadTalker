...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=False):
        ...
        parser.add_argument(
            "--bfm_folder", type=str, default="./checkpoints/BFM_Fitting/"
        )
        parser.add_argument(
        ...
        if is_train:
            ...
            parser.add_argument(
                "--shift_pixs", type=float, default=10.0, help="shift pixels"
            )
            parser.add_argument(
                "--scale_delta", type=float, default=0.1, help="delta scale factor"
            )
            parser.add_argument(
                "--rot_angle", type=float, default=10.0, help="rot angles, degree"
            )

            # loss weights
            parser.add_argument(
                "--w_feat", type=float, default=0.2, help="weight for feat loss"
            )
            parser.add_argument(
                "--w_color", type=float, default=1.92, help="weight for loss loss"
            )
            parser.add_argument(
                "--w_reg", type=float, default=3.0e-4, help="weight for reg loss"
            )
            parser.add_argument(
                "--w_id", type=float, default=1.0, help="weight for id_reg loss"
            )
            parser.add_argument(
                "--w_exp", type=float, default=0.8, help="weight for exp_reg loss"
            )
            parser.add_argument(
                "--w_tex", type=float, default=1.7e-2, help="weight for tex_reg loss"
            )
            parser.add_argument(
                "--w_gamma", type=float, default=10.0, help="weight for gamma loss"
            )
            parser.add_argument(
                "--w_lm", type=float, default=1.6e-3, help="weight for lm loss"
            )
            parser.add_argument(
                "--w_reflc", type=float, default=5.0, help="weight for reflc loss"
            )
