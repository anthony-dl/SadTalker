...
def main(args):
    ...
    # audio2ceoff
    batch = get_data(
        first_coeff_path, audio_path, device, ref_eyeblink_coeff_path, still=args.still
    )
    ...
    saved_video_path = os.path.join(args.result_dir, image_name + ".mp4")
...
def inference_api(
    ...
    processing_time = et_time - st_time
    print(f"Proceessing the whole pipeline takes {processing_time}")
